...
|---|---|---|---|---|---|
| A1 | Backend: Discord OAuth2 endpoints | Implement `GET /api/auth/login`, `GET /api/auth/callback`, `POST /api/auth/logout`, `GET /api/auth/me`. Authorization Code Grant flow with `identify` scope only. Exchange code for token, fetch `/users/@me`. Guild membership is verified server-side via the bot sidecar (`get_member()` lookup), not via the Discord user API. Match to Member by `discord_id`, issue signed JWT in HttpOnly/Secure/SameSite=Lax cookie. CSRF state in short-lived cookie. | `backend`, `auth`, `v1.0` | L | None |
